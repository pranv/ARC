...
class ConvARC3DA(lasagne.layers.Layer):
	def __init__(self, incoming, num_filters, lstm_states, image_size, attn_win, 
		...
		W_g = normal_init(shape=(5, lstm_states), sigma=0.01)

		self.W_lstm = self.add_param(W_lstm, (4 * lstm_states, num_input + lstm_states + 1), name='W_lstm')
		self.W_g = self.add_param(W_g, (5, lstm_states), name='W_g')
	...
	def get_output_for(self, input, **kwargs):
		...
		def step(glimpse_count, c_tm1, h_tm1, odd_input, even_input, W_lstm, W_g):
			...
			flat_glimpse = glimpse.reshape((B, attn_win ** 2))
...
if __name__ == "__main__":
	...
	l_conv = Conv2DLayer(l_in, num_filters=4, filter_size=(3, 3), stride=(1, 1), pad='same')
	l_carc = ConvARC3DA(l_conv, num_filters=4, lstm_states=5, image_size=7, \
